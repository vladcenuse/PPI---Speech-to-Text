...
// Mapping function to convert field keys to Romanian labels
const getRomanianLabel = (key, documentId) => {
  ...
  const fieldMappings = {
    ...
    'prescription-form': {
      ...
      'dosage': 'Dozaj Recomandat',
      'instructions': 'Instrucțiuni',
      'followUp': 'Instructiuni de Urmarire'
    },
    'medical-report': {
      'chiefComplaint': 'Plângere Principală',
      'historyOfPresentIllness': 'Istoricul Prezent',
      'physicalExamination': 'Examinare Fizică',
      'diagnosis': 'Diagnostic',
      'treatment': 'Tratament',
      'recommendations': 'Recomandări'
    },
    'echocardiography-form': {
      'aorta_la_inel': 'Aorta la Inel',
      'aorta_la_sinusur_levart_sagva': 'Aorta la Sinusur Levart Sagva',
      'aorta_ascendenta': 'Aorta Ascendenta',
      'ventricul_drept': 'Ventricul Drept',
      'atriu_stang': 'Atriu Stang',
      'as': 'AS (Aorta Ascendentă)',
      'vd': 'Vd'
    },
    'new-patient-form': {
      ...
      'chiefComplaint': 'Plângere Principală',
      'presentIllness': 'Istoricul Prezent',
      'pastMedicalHistory': 'Istoric Medical Trecut',
      'medications': 'Medicatie Curenta',
      'allergies': 'Alergii',
      'familyHistory': 'Istoric Familial',
      'socialHistory': 'Istoric Social',
      'vitalSigns': 'Semne Vitale',
      'physicalExam': 'Examinare Fizică',
      'assessment': 'Evaluare',
      'plan': 'Plan de Tratament',
      'followUp': 'Instructiuni de Urmarire'
    }
  ...
  // Get the mapping for this document type
  const mapping = fieldMappings[documentId] || {}
  
  // Return mapped label or capitalize the key as fallback
  if (mapping[key]) {
    return mapping[key]
  }
  
  // Fallback: try to convert camelCase/snake_case to readable Romanian
  const converted = key
    .replace(/([A-Z])/g, ' $1')
    .replace(/_/g, ' ')
    .toLowerCase()
...
const exportRecordToWord = (record) => {
  ...
  let content = `
    ...
    <body>
      <div class="header">
        <h1>${documentName}</h1>
        <div class="date">Data: ${new Date(record.date).toLocaleDateString('ro-RO')}</div>
  ...
  if (record.data) {
    Object.entries(record.data).forEach(([key, value]) => {
      if (value && value.toString().trim() !== '' && key !== 'patientId' && key !== 'date') {
        const label = getRomanianLabel(key, documentId)
        content += `
          <div class="field">
  ...
  content += `
      <div class="footer">
        <p>Document generat de sistemul Speech-to-Text Medical</p>
  ...
  toastService.success(
    'Document Word Descărcat',
    'Fișierul a fost salvat. Deschideți-l cu Microsoft Word sau un alt procesor de text compatibil.'
  )
}
